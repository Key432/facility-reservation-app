...
注意点として、`count`の変更は即時には反映されません。以下のように書いた場合、数値が3ずつ増えるわけではありません。
...
レンダリングのタイミングはパフォーマンスやデータの取得タイミングに依存するため、コンポーネント設計において重要です。
...
以下は、親コンポーネントから受け取った名前を用いてPoke APIからポケモンの情報を取得するコンポーネントの例です。
...
`useEffect()`の第二引数に渡される配列を依存配列と呼びます。この配列に指定された変数が変更されたとき、またはコンポーネントが最初にレンダリングされたときに、第一引数の関数が実行されます。この例では、`name`が変更されるたびに新しいポケモン情報がフェッチされます。依存配列に何も指定しない（空の配列`[]`を渡す）場合、Effectはコンポーネントのマウント時にのみ実行されます。
